...
const returnIpsum = (num) => {
    ...
    let stringList = [];
    let assignMeSomething = xzibit_Isims.slice(0);
    ...
    function newIpsum() {
        ...
        for( var i = 0; i <= rand; i++){
            ...
            arrayHolder.push(randomArrayValue);
        }
    ...
    if(100 < num){
        ...
        console.log('the number is waaay too long');

    } else if(num == "" || num == null) { 

        for(var j = 0; j < 5; j++){
            let updtstr = newIpsum();
            console.log(updtstr);
            stringList.push(updtstr);
            assignMeSomething = [];
